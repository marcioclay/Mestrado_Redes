...
def WorkSelectRoom(device, msg, controlQueue):
	...
	if roomItem != None:
		deviceID = NewID(lock)
		device.ID = deviceID
		device.value = -1 # Modificado para -1 para aceitar receber o valor 0 (Desligar) na primeira vez
		...
		# --- MODIFICADO: Aceita Lâmpada e Ar-Condicionado ---
		if device.typeCode == COD_LAMPADA or device.typeCode == COD_AR_CONDICIONADO:
			...
			# Usa COD_LAMPADA na fila para o GeneralControl repassar a mensagem automaticamente
			controlQueue.put(MonitorItem(device.ID, COD_LAMPADA, device.roomID, INCLUIR_LAMPADA, device.lampQueue))
...
def WaitLampQueue(device):
	print(f'Aguardando evento, atuador {device.ID}')
	while True:
	...
	# --- MODIFICADO: Permite comando de climatização ---
	if action >= 0:
		if action == LUZ_ACESA:
			print(device.toString() + ': Acender lâmpada')
		elif action == LUZ_APAGADA:
			print(device.toString() + ': Apagar lâmpada / Desligar Ar-Condicionado')
		elif action == 23:
			print(device.toString() + ': Ar-condicionado SET 23 graus')
			
		msg = MessageLamp()
		SendMessage(device, msg.pack(device.ID, action))
	else:
		print(device.toString() + ': Comando inválido')
		msg = MessageStatus()
...
def WorkLamp(device, msg):
	...
	if msg.code == MSG_STATUS:
		if msg.status == ACAO_EXECUTADA:
			print(device.toString() + ': Ação executada.')
	else:
...
def WorkSensor(device, msg, controlQueue):
	...
	# --- MODIFICADO: Regra Única de Climatização ---
	elif device.typeCode == COD_TERMOMETRO:
		if device.value >= 26.0:
			print('Temperatura >= 26. Reduzindo Ar-Condicionado para 23.')
			# Usa COD_SENSOR_PRESENCA na fila para o GeneralControl repassar a ação automaticamente
			controlQueue.put(MonitorItem(device.ID, COD_SENSOR_PRESENCA, device.roomID, 23, None))
		else:
			print('Temperatura < 26. Desligando Ar-Condicionado.')
			controlQueue.put(MonitorItem(device.ID, COD_SENSOR_PRESENCA, device.roomID, 0, None))
...
def DeviceThread(connection, clientIP, controlQueue):
	...
	if device.ID != None:
		if device.typeCode == COD_LAMPADA or device.typeCode == COD_AR_CONDICIONADO:
			controlQueue.put(MonitorItem(device.ID, COD_LAMPADA, device.roomID, EXCLUIR_LAMPADA, device.lampQueue))
